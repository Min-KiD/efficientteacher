...
from utils.general import check_dataset, check_requirements, check_yaml, clean_str, segments2boxes, \
    xywh2xyxy, xywhn2xyxy, xyxy2xywhn, xyn2xy, xyn2xy_new, colorstr
...
class LoadImages:
    ...
    def __next__(self):
        ...
        # print('/home/bowen/quan_img/' + path.split('/')[-1])
        # print(img.shape)
        # cv2.imwrite('/home/bowen/quan_img/' + path.split('/')[-1], np.ascontiguousarray(img))
        # import pdb
        # pdb.set_trace()
        # Convert
...
class LoadImagesAndLabels(Dataset):
    ...
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        #id逻辑与points逻辑分离开
        if self.with_id:
            self.ids = [l[:,5] for l in self.labels]
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            ...
            if labels.size:  # normalized xywh to pixel xyxy format
                if self.with_id: #对带有跟踪id的标注进行变换
                    labels[:, 1:5] = xywhn2xyxy(labels[:, 1:5], ratio[0] * w, ratio[1] * h, padw=pad[0], padh=pad[1])
                    id_value = self.ids[index].copy()
                    id_value = id_value.reshape(-1, 1)
                    # print('id_value:', id_value)
                    labels = np.concatenate((labels, id_value), 1)
                else:
        ...
        if nl:
            ...
            if self.with_id:
                labels[:, 1:5] = labels[:, 1:5].clip(0, 1.0)
            else:
    ...
    def showlabels(self, img, boxs, ids, landmarks, path):
        ...
        for i,box in enumerate(boxs):
            ...
            cv2.putText(img, str(int(ids[i])), (int(x - w/2)-10, int(y-h/2)-10), cv2.FONT_HERSHEY_SIMPLEX, 120/ (150), (0, 255, 0), round(240 / 150))
        if landmarks != []:
    ...
    for i, index in enumerate(indices):
        ...
        if self.with_id: #对带有跟踪id的标注进行变换
            if labels.size:
                ...
                #     id_pad = np.ones((labels.shape[0], 1)) * -1
                #     labels = np.concatenate((labels, id_pad), 1)
...
def cal_cur_max_id(lb_files, pseudo_labels=False):
    ...
    for lb_file in lb_files:
        if os.path.isfile(lb_file):
            id_key_tmp = lb_file.split('/')[-2]
            id_list_image = []
            if id_key_tmp != id_key:
                id_key = id_key_tmp
                id_start_index = id_cur_max + 1
                print('id_start_index:', id_start_index)
                print('id_key:', id_key)
                print('id_key_tmp:', id_key_tmp)
            with open(lb_file, 'r') as f:
                l = [x.split() for x in f.read().strip().splitlines() if len(x)]
                for x in l:
                    #发现当前标注中没有跟踪id标号,按顺序分配假id
                    if pseudo_labels:
                        ...
                        if len(x) < 6: 
                            ...
                            id_new = int(x[5]) + id_start_index
                    print(x, len(x))
                    print('id_new:', id_new)
                    id_cur_max = max(id_new, id_cur_max)
